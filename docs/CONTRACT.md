...
**Owner:** Story 5.1 (PI data model), 5.2 (log a supplier's confirmed PI), 5.3 (variance calculation), 5.4 (tolerance evaluation & auto-confirm). Still to come: approval routing (5.5/5.6) and the full status lifecycle/audit trail (5.7) — see "Still to come" below.
...
**Nothing open blocks the reconciliation core anymore.** The variance basis (unit price) and the ±2% tolerance boundary (exclusive, per-account 2%) are both resolved and built — pending only Product Owner confirmation the two defaults are right, which is a value change, not a rebuild. 5.5+ (the approval workflow) has the inputs it needs.
