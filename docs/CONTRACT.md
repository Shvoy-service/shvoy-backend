...
**Owner:** Story 4.7 (the seam), Story 9.4 (SES goes live). `com.shvoy.EmailSender` (interface), `EmailMessage`/`EmailAttachment` (the message shape), `ConsoleEmailSender` (local/test) and `SesEmailSender` (dev/prod).

- **The seam is now live — profile-selected, same pattern as the identity provider.** `local`/`test` keep `ConsoleEmailSender` (logs the message, no real send — frontend devs and tests don't send mail); `dev`/`prod` use `SesEmailSender` (Amazon SESv2, `eu-west-2`). **Zero consumer changes** to the sending behaviour was the swap's proof — the only touch was tagging each message's `source` + `entityReference` for the send record (metadata, not coupling). Four consumers: `InvitationService` (2.3), `PurchaseOrderSendService` (4.7, PO PDF attachment), `ApproverNotifier` (5.5), `DiscrepancyNotifier` (6.6). SES simple content carries the PDF attachment natively (no raw MIME); the ECS task role's scoped `ses:SendEmail` authenticates it (no SMTP, no credentials in code) — see `docs/aws/A9-ses-email.md`.
- **Failure isolation is the one real commitment.** The consumers call `send` with **no try/catch**, so the sender must **never throw**: `SesEmailSender` catches every SES failure (down, sandbox-unverified recipient, throttle), classifies it, records it, and swallows it — the business action it accompanies always completes (the invite still creates). Synchronous send, **no retry/queue machinery** (a failed send's record is the retry mechanism — a human resends).
- **Every attempt is recorded — `send_records` (V72), append-only, tenant-scoped** (9.6 folded in): `source`, `recipient`, `subject`, `outcome` (`SENT` / `FAILED_PERMANENT` / `FAILED_TRANSIENT` — the permanent/transient split is the data a future retry reads), SES message id or error, `entity_reference`, timestamp. **Never the email body** — subject and metadata only; bodies carry invite links and the record must not become a token store (2.3's token-hygiene rule, extended to the transport). The record write is itself best-effort (`REQUIRES_NEW`, swallowed on failure) so it can't break the send either.
- **Sandbox reality (dev):** a new SES account sends only to *verified* recipients, so an unverified dev recipient produces a real `FAILED_PERMANENT` — which the isolation above turns into a recorded failure, not a broken invite. Production sending needs the SES sandbox-exit request (lead time) — a prod-infrastructure-story item, noted in A9, not built here. Email content/templates are 9.5 (the stub strings send as-is until then).
- **Deliberately not touched:** `RegistrationService`'s own verification-email log line — the same pattern but never migrated onto `EmailSender` (out of 4.7's named scope); a natural future consumer, not an oversight.
...
| `CONSIGNMENT_NOT_ARRIVAL_ELIGIBLE` | 409 | Arrival confirmation attempted on a consignment that isn't `PROVISIONALLY_RECEIPTED` (no GRN to confirm against) — see Physical arrival confirmation below |
| `ARRIVAL_ALREADY_CONFIRMED` | 409 | Arrival confirmation attempted on a consignment whose arrival is already confirmed — see Physical arrival confirmation below |
| `ARRIVAL_NOT_CONFIRMED` | 409 | Arrival-date correction attempted on a consignment whose arrival isn't yet confirmed — see Physical arrival confirmation below |
| `ETD_NOT_SETTABLE_AFTER_ARRIVAL` | 409 | Confirmed-ETD set attempted on a consignment whose arrival is already confirmed (an ETD after landing is moot) — see ETD tracking below |
...
**Owner:** Story 3.3, **remodelled by the supplier-record remodel** (confirmed PO terms model). The flat single-record-per-supplier structure is replaced by **typed term records** a supplier references by id (a `current` term, an optional `target`, and retained history). Each term carries `terms_type` (`ZERO_DEPOSIT`/`DEPOSIT_BALANCE`/`ROLLING`), a nullable `deposit_pct` (1dp, populated only for `DEPOSIT_BALANCE`), a **five-value** `anchor_date_type` (BL/INVOICE/EX_FACTORY/ARRIVAL/**STATEMENT_DATE**), and a signed `days_from_anchor`. **Type-consistency is enforced** (`INVALID_TERMS_COMBINATION`): `DEPOSIT_BALANCE` needs 0<pct<100 (an explicit 0 is `ZERO_DEPOSIT`, not two spellings of one fact); `ZERO_DEPOSIT`/`ROLLING` need a null deposit; a `STATEMENT_DATE` anchor is coherent only for `ROLLING`. **Target activation** (`POST .../payment-terms/target/activate`, `ADMIN`/`FINANCE`, audited) promotes target→current and retains the old current historically — the activation *mechanism* is explicit-action regardless of whether the POs later add a date-based trigger (which would just call it; flagged). **`ROLLING` is storable end-to-end here; the terms-type-aware payment behaviour (rolling suppliers not spawning per-PO payments) is deliberately the 6.5 re-spec, out of scope.** **The 6.2 resolver (`PaymentTermsService#getScheduleTerms`/`trySplit`) is the isolation point — it now reads the `current` term and no caller changed**, exactly the payoff the resolver was built for. Existing terms migrate via Flyway transform (V59, reusing each supplier's id as its term id — portable, no SQL UUID generation): deposit 0→`ZERO_DEPOSIT`, else `DEPOSIT_BALANCE`. Existing POs' snapshotted due dates are untouched (they live on payment rows, never referencing terms — pinned by a test that changes a supplier's current terms and proves an in-flight payment's due date doesn't move).

This story stores terms and defines the split rule; it does not calculate actual due dates against a real order — that's Feature 7.
...
### Invoice logging (Story 6.4, remodelled — invoice remodel)

The fourth leg of the three-way match — the supplier's final invoice (PO = PI = GRN vs **invoice**). Same "record faithfully, judge later" posture as PI logging (5.2): capture what the supplier sent; the match (6.5) judges it. In the `payments` module. The remodel (the last model change before the 6.5 match re-spec) settles the flagged cardinality question — a PO now carries **many concurrent invoices** — and adds a mandatory **`covers_type`** and a derived **running position**.

| Method | Path | Role | Notes |
|---|---|---|---|
| `POST` | `/api/purchase-orders/{poId}/invoices` | ADMIN/PURCHASING/FINANCE | Record a **new** invoice (`{ invoiceReference, amount, currency, invoiceDate, coversType, coversConsignmentId?, coveredLines?, claimedCreditAmount?, claimedCreditReference? }`). `PO_NOT_READY_FOR_INVOICE`/409 unless the PO is `GENERATED`/`SENT`. **Logging never supersedes** — the new invoice is simply active alongside any others. |
| `POST` | `/api/invoices/{id}/corrections` | ADMIN/PURCHASING/FINANCE | Correct **one specific** invoice: supersedes `{id}` (kept for audit, `SUPERSEDED`) and records its replacement, chained via `supersedes_invoice_id`. The only path that supersedes anything. Same request body as logging. |
| `GET` | `/api/purchase-orders/{poId}/invoices` | Any authenticated | Every invoice logged against a PO — active + superseded — newest first. |
| `GET` | `/api/invoices/{id}` | Any authenticated | One invoice (with its `coversType`, `coveredLines`, `supersedesInvoiceId`, and `weakestSignal`). |
| `GET` | `/api/purchase-orders/{poId}/running-position` | Any authenticated | The PO's derived running position (see below). |

- **Many invoices per PO (cardinality change).** The prior "one active invoice, a second supersedes all" rule is gone — real suppliers invoice a PO in **parts** (a deposit invoice + a per-shipment invoice + a lines invoice, all live at once). Supersession is now a **targeted correction of one specific invoice/chain** (`InvoiceRecordingService#recordCorrection`), not "replace the PO's invoice". One logical invoice = one supersession chain (`supersedes_invoice_id`, a self-referential column — same flat-FK convention as `purchase_order_id`).
- **`covers_type` — mandatory, five values, coherence-checked at entry.** What the invoice *declares* it covers, validated for **well-formedness only** (existence / ownership / PO-shape coherence — `InvoiceCoverageValidator`), never for whether the amount agrees with the coverage (that's the match). A failed coherence check is `INVOICE_COVERAGE_INCOHERENT`/409.
  - `DEPOSIT` / `BALANCE` — the PO must carry the matching obligation (a `DEPOSIT` / `BALANCE` payment exists); a deposit invoice against a zero-deposit / rolling PO is incoherent.
  - `SHIPMENT` — carries a `coversConsignmentId` that must name a consignment **receipted against this PO** (present in the GRN projection). The **strongest** reconciliation signal.
  - `LINES` — carries `coveredLines` (`[{ skuId, quantity }]`); every claimed SKU must belong to the PO. Claimed quantities are recorded, judged by the match.
  - `AMOUNT` — the free-standing **fallback**, no breakdown. **Never rejected**, but flagged `weakestSignal: true` on entry and on every view — the weakest reconciliation signal, deliberately not the default path.
- **Record faithfully.** Beyond coverage coherence, validation is well-formedness only (non-blank reference, positive 2dp amount, real ISO 4217 currency, a date). Amount/currency **disagreement with the PO/PI is recorded, not rejected**; a differing currency is recorded with **no FX conversion** — the match flags it.
...
- **One internal operation** (`InvoiceService#log` / `#correct`) for manual entry now and the AI feed later, same as 5.2.
- **The anchor seam under many invoices — one isolated policy point (flagged interpretation).** `InvoiceAnchorPolicy` decides whether recording an invoice (re-)publishes the `INVOICE` `AnchorEventDateKnownEvent`: the anchor publishes from the **first non-deposit invoice** — a deposit invoice never anchors a balance due date, and a later independent invoice (a second shipment) never moves the anchor the first one set. A **correction of that first non-deposit invoice's chain does re-fire** (its date may have changed). `PaymentDueDateService` (6.2) reacts as before; best-effort, a trigger failure never fails the logging. *This is the single knob that changes if the PO owner reads the anchoring rule differently — deliberately isolated, not smeared across the service. Moot for STATEMENT_DATE / rolling POs.*
- **The running position — one derived, shared, read-time-computed operation** (`RunningPositionService`, three consumers: the 6.5 match re-spec, the Finance view, statement reconciliation). Returns **% invoiced / % paid / % received** plus the underlying money and an `overInvoiced` flag. Bases (pinned): PO value = Σ line qty × **snapshot** price; invoiced = Σ **active** invoice amounts; paid = Σ `PAID` payment amounts; **received = Σ GRN-projected qty × the PO's snapshot price for that SKU** (quantity × PO price — never the invoice's price). **Never cached** — a stored percentage would drift the first time a supersession or GRN amendment landed, so every number is recomputed on each read. **Divergence between the three percentages is normal and never validated** (an invoice can precede goods, goods can precede payment). The one boundary surfaced is `overInvoiced` (cumulative invoiced > PO value): **warn-and-surface, not a block** — the hard over-invoice rule is a pending PO answer, deferred to 6.5.
- **Migration.** Legacy (one-per-PO) invoices migrate to `covers_type = AMOUNT` — what the data *says* (no coverage was ever captured), not what it probably *meant*; being `AMOUNT` (the weakest signal) is itself the flag for manual reclassification. Never invent coverage in a migration. Flyway V65 (`covers_type` default `AMOUNT` + `covers_consignment_id` + `supersedes_invoice_id`), V66 (`invoice_covered_lines`).
...
| `GET` | `/api/credit-ledger/stats` | Any authenticated | `{ openCount }` — the count of **open credits** (a ledger view). *Not* the Screen-1 "Open discrepancies" stat — that's open discrepancy **cases** (6.6, `GET /api/discrepancies/stats`); this was reconciled there. |
...
### Three-way match evaluation (Story 6.5, re-spec for the 1:many world)

The pre-payment control the product exists for, rebuilt once invoices became many-per-PO with declared coverage (6.4 remodel). **Two dispatches that compose:** each active invoice matches against **what it declares it covers** (`covers_type` → the strategy), and the verdict's *consequence* depends on the supplier's `terms_type` (per-PO payment gating vs statement-feeding). Block-by-default stands. In `payments`. The endpoint `GET /api/purchase-orders/{poId}/match-results` returns the per-invoice verdicts (what Finance and, later, the statement view read).

> **The architectural spine:** *strategy* (what "matches" means) and *policy* (what a match *does*) are separate dispatches — `covers_type` picks the first (`InvoiceMatchEvaluator`), `terms_type` picks the second (`MatchConsequencePolicy`). They compose freely: a rolling supplier's shipment invoice uses the *same* strategy as a deposit/balance supplier's; only the consequence differs. Everything left in the backlog (statement reconciliation, freight, NCR) plugs into one or the other.

- **Universal preconditions.** Every invoice match needs a **confirmed PI** (`AUTO_CONFIRMED`/`APPROVED` only — pulled via `ProformaInvoiceMatchService`, `@NamedInterface("reconciliation")`) plus the legs its strategy needs. Missing legs → the invoice is *unmatchable* ("awaiting X"), honestly distinct from `BLOCKED`.
- **The five strategies — dispatch on `covers_type`** (pure `InvoiceMatchEvaluator`, Spring-free & unit-tested). Exact 2dp money equality throughout *except* `AMOUNT`:
  - **`DEPOSIT`** — expected = the snapshotted deposit amount (6.1). Legs: PO + PI, **no GRN** (deposits precede shipment by design; the deposit invoice's match *is* the deposit's gate).
  - **`BALANCE`** — expected = the snapshotted balance amount, minus validated credits. Legs: PO + PI + **cumulative** GRN, and it matches only when **received-complete** (cumulative received = ordered) — a balance invoice on a half-shipped PO fails "receipt incomplete" (the short-shipped-but-fully-invoiced catch, now cumulative).
  - **`SHIPMENT`** — the strongest strategy: expected = the referenced GRN's received quantities × confirmed-PI prices, minus validated credits. Legs: PO + PI + that specific consignment's GRN. `qc_failed` on the GRN never blocks (7.4's confirmed rule).
  - **`LINES`** — expected = claimed quantities × PI prices, each claim **capped at the cumulative receipt of that line** (can't match more of a line than has arrived).
  - **`AMOUNT`** — the fallback: no declared coverage, so it **position-matches** — it passes only if it fits within the still-**unclaimed** received value (cumulative received value − already-matched invoice value). Flagged `positionMatched` on the verdict so Finance sees it reconciled loosely; never a substitute for a coverage strategy.
- **Credit (6.7, unchanged).** A claimed credit must match an OPEN ledger entry (`checkClaim`) — matched → deducted from expected and the entry `APPLIED` on a pass; unmatched → fail. Re-entrant: an already-applied credit is recognised on a re-run (`claimAlreadyAppliedToInvoice`).
- **The rollup — PO-level coherence over per-invoice verdicts** (isolated `MatchRollupEvaluator`, run after a strategy passes, before the verdict commits). Per-invoice matching created a new failure mode: invoices each individually plausible yet **collectively over-claiming**. An invoice fails the rollup if letting it through would push cumulative matched value past *either* cumulative received value + the deposit obligation, *or* the PO's own coverage (its ordered value). **The over-invoice bound — warn-and-surface at entry (6.4) — becomes a hard fail here.** Invoices are evaluated in a stable order (created, then id) accumulating matched value, so the rollup is deterministic about which one fails.
- **The consequence — dispatch on `terms_type`** (`MatchConsequencePolicy`, the one isolated policy point; terms resolved live via `PaymentTermsService#getEffectiveTermsType`, isolated so it can become a snapshot later):
  - **`DEPOSIT_BALANCE` / `ZERO_DEPOSIT`** (and an unconfigured supplier — block-by-default): a passing match flips the corresponding payment (`DEPOSIT` invoice → deposit; a passing `BALANCE` invoice → balance) to `READY_TO_PAY`; any failing non-deposit invoice → the balance `BLOCKED` + a discrepancy case (6.6, unchanged). Partial `SHIPMENT`/`LINES`/`AMOUNT` invoices can pass without releasing the balance — the balance is claimed by a `BALANCE` invoice. The deposit stays **payable-without-match** until a `DEPOSIT` invoice is logged (deposits precede everything).
  - **`ROLLING`** — the match runs **identically** as a *document control*: every verdict recorded, discrepancy cases open on failure (a rolling supplier can still short-ship), but **no per-PO payment transition** — the verdicts feed the statement view (the next-but-one story). Recorded as `policy_applied = STATEMENT_RECORDED`.
- **Durable per-invoice verdict.** Each evaluation upserts an `invoice_match_results` row (V67) — covers/terms type, pass, `positionMatched`, expected vs invoice amount, detail, `policy_applied`. One current row per invoice; it carries the verdict independent of any payment transition, which is exactly what a rolling supplier needs.
- **Event-driven.** `MatchTriggerListener` re-evaluates on every input arrival/change — PO generated, PI confirmed, GRN created/amended (**including a new partial shipment, which can flip a waiting `BALANCE` invoice to matchable via cumulative receipt**), invoice logged/superseded, credit changes. All `@TransactionalEventListener(AFTER_COMMIT, fallbackExecution=true)`; `ThreeWayMatchService#evaluate` runs `REQUIRES_NEW` (an after-commit `REQUIRED` write silently doesn't persist). Deterministic; the balance's `match_detail` (V53) carries the per-invoice failure detail; human `PAID`/`ON_HOLD` never overridden.

#### The Feature 5/7 → 6 seam is push, not pull

Both cross-module legs the match pulls-or-reacts-to are shaped by the module graph:
- **GRN (from `shipments`): push + projection.** `shipments` already depends on `payments` (the anchor event), so a `payments → shipments` pull would cycle. So payments **projects** the received quantities from 7.4's `ProvisionalGoodsReceiptEvent` into a local table (`payment_grn_projection_lines`, V52) and the match reads the projection. (This means creating a GRN now writes a payments-side row — a cross-module test-cleanup coupling, same class as "generation creates payment rows".)
- **Confirmed PI (from `reconciliation`): trigger event + pull.** No `reconciliation ↔ payments` dependency existed, so the confirmed-PI trigger is a `reconciliation`-owned event and payments *also* pulls the PI detail via `ProformaInvoiceMatchService` — both a single `payments → reconciliation` direction, no cycle (the `PurchaseOrderGeneratedEvent` shape).

### Receipt rollup & PO closure (Story — cumulative receipt + the closure lifecycle)

Confirmed Q3: per-PO cumulative receipt across many shipments ("all rolling up to the parent PO"), and closure when **cumulative received = ordered**. The cumulative *operation* lives with the receipts (`shipments`); the closure *state* lives on the PO (`purchaseorders`). Spans both, with the receipt side observing the fact and the PO owning the transition.

- **The cumulative-receipt operation — one owner (`ReceiptRollupService`, `shipments`).** Given a PO, its per-SKU received quantities summed across every consignment's GRN (7.4 snapshots) and the derived received *value* (quantities × PO snapshot prices — the basis 6.4 pinned). **Derived at read time, never stored counters** (the drift rule — a GRN amendment reflects instantly), same shape as the running position. Read via `GET /api/purchase-orders/{poId}/receipt-rollup` (per-SKU ordered/received/valued + `complete`/`overDelivered`/outstanding). **Module-graph note:** `payments` can't depend on `shipments` (cycle), so 6.5 / the running position keep reading their own `payment_grn_projection_lines` copy — the sanctioned cross-boundary projection of the same GRN snapshots. This service is the canonical owner on the receipt side; both derive from the same 7.4 event, so they agree.
- **Closure — an observed fact, not a command.** New PO statuses `CLOSED` / `CLOSED_SHORT` (extending the Feature 4 enum). On every receipted/amended event, `shipments` re-assesses and pushes the result to `purchaseorders` (`PurchaseOrderService#applyReceiptClosure`, its `@NamedInterface` — shipments→purchaseorders already exists): complete → `SENT`/`GENERATED` → `CLOSED` (audited system action); a later amendment that un-completes it → `CLOSED` → `SENT` (reopened **loudly** — silent wrong-closure is worse). **Per-SKU exactness is the load-bearing rule:** complete means every line met exactly — a shortfall on one SKU netting against an overage on another is *not* complete, it's a double discrepancy (a dedicated test guards this).
- **What `CLOSED` gates — and doesn't.** It blocks new consignments / shipment documents (they run through `assertOwnPurchaseOrderReadyForShipment`, which only accepts `GENERATED`/`SENT`). It does **not** freeze the financial side: invoices still arrive (the balance invoice typically *follows* completion — `assertOwnPurchaseOrderReadyForInvoice` now also accepts `CLOSED`/`CLOSED_SHORT`), matches run, payments transition, cases resolve. Closure is a receipt-side fact, not a done-with-everything flag — pinned by tests.
- **Over-delivery — isolated, interim (pending the PO answer).** Cumulative received exceeding ordered on a SKU: the GRN still **creates** (goods are physically real, same posture as `qc_failed`), the PO is flagged `over_delivered` (V68, surfaced on the PO + the rollup view per SKU), and the PO does **not** close (it *exceeds* — a discrepancy to resolve, not completion). One isolated evaluator so the eventual rule (hard-block / tolerance / auto-accept) is a contained change.
- **Short-closure — the manual escape valve (`FINANCE`/`ADMIN`).** `POST /api/purchase-orders/{poId}/close-short` (in `shipments` — it computes the outstanding), required reason, → the **distinct** `CLOSED_SHORT` state ("complete" and "abandoned remainder" are different facts the reporting/audit story cares about). Fully audited (who / why / outstanding at closure). Finance-grade authority, same segregation as the 6.6 override — Purchasing is rejected. Auto-suggesting a credit for the undelivered value is deferred (pending); the outstanding value is surfaced, nothing created.

### Mismatch routing & resolution (Story 6.6)

The human half of the match control: when 6.5 blocks a payment, a **discrepancy case** puts the mismatch in front of a named resolver with the comparison side-by-side, and gives the paths to resolve it. In `payments`. Endpoints under `/api/discrepancies`: `GET` (queue), `GET /stats`, `GET /{id}` (side-by-side), `POST /{id}/claim` / `/credit` / `/override` / `/dispute`.

- **Case creation is driven by the match, idempotently.** 6.5's `evaluateBalance` calls `onMatchBlocked` (open a case, or refresh an existing one's detail — one case per blocked payment, never duplicates) and `onMatchPassed` (auto-resolve). `discrepancy_cases` (V54) + append-only `discrepancy_case_audit_events` (V55, the case's paper trail). A block notifies via the shared `EmailSender` seam — the **fourth** consumer (invites, PO send, approvals, now discrepancies) — addressing the claimable queue via a new onboarding surface `UserDirectoryService#resolveDiscrepancyResolverEmails` (`@NamedInterface`, active `PURCHASING`/`ADMIN`).
- **Routing = a claimable queue** (flagged interpretation of "named resolver"): any `PURCHASING`/`ADMIN` user claims a case and their name goes on the record — no assignment machinery built speculatively (a per-company default resolver is the natural later addition).
- **The side-by-side is served whole** (`GET /{id}`): every leg's current value (PO qty/price, PI qty/price, GRN received qty, invoice amount + claimed credit and its ledger verdict) plus the PO's **open ledger entries** that may explain the variance — the client assembles nothing. Read fresh (the resolver wants the *current* comparison), never re-running the verdict.
...
## Shipment & Goods Receipt (Feature 7)

**Owner:** Story 7.1 (shipment data model). This is the feature the blocked half of Feature 6 waits on — 7.4's provisional GRN is what the three-way match (6.5), mismatch routing (6.6), and release (6.8) run against. 7.1 is model-only: entities and schema, no upload flow, no anchor publishing, no GRN creation, no arrival logic (those are 7.2–7.6).

### The two-entity split (Story 7.1)

The structural decision the whole feature rests on. The naive model — one shipment per PO, BL as fields on it — is broken by the co-loading rule: *"a single BL can be linked to multiple POs across multiple suppliers; each linked PO requires its own packing list before its portion can be receipted."* So there are genuinely two concepts, modelled as two entities:

- **`Shipment`** (table `shipments`, V44) — the BL/container-level record: `blReference`, `blDate`, `exFactoryDate`, and the BL document's S3 key. One physical shipment, one Bill of Lading, potentially carrying goods for several POs.
- **`ShipmentConsignment`** (table `shipment_consignments`, V45) — one PO's portion of a shipment: `shipmentId`, `purchaseOrderId`, its **own** packing-list and inspection-report S3 keys (per-consignment, not per-shipment — the co-loading rule is explicit), `confirmedEtd`, `arrivalDate`, and `receiptStatus`.

**A non-co-loaded shipment is simply a shipment with one consignment** — no special-casing, the common case costs nothing extra. **Co-loaded is derived, not stored**: a shipment is co-loaded when it has more than one consignment; callers count consignments (7.3), there is no persisted flag. Do not flatten the two entities into one "for simplicity" — the simplicity would be borrowed against a certain remodel in 7.3, since every downstream rule (per-consignment packing lists 7.3, per-portion receipting 7.4, arrival-vs-provisional per portion 7.6) lands on this shape.

**Both entities are `TenantScoped`**; the PO reference is a loose UUID + DB foreign key, not a JPA association — the same convention every entity here follows (e.g. `Invoice#purchaseOrderId`); modules share ids and events, never object graphs (`ModularityTests` stays green — `shipments` gained no cross-module dependency).

### The receipt lifecycle (`ReceiptStatus`)

Per-consignment, because each portion is receipted independently: `DOCUMENTS_PENDING` (every consignment's start) → `PROVISIONALLY_RECEIPTED` (the provisional GRN exists, 7.4 — explicitly *not* requiring physical arrival) → `ARRIVED_CONFIRMED` / `ARRIVED_WITH_DISCREPANCY` (7.6, where a mismatch spawns a 6.7 credit-ledger discrepancy, never a reopened payment). States defined now; the **transitions are not yet enforced** — the stories that own each step (7.4, 7.6) enforce them.

### Anchor dates live here but publish later

`blDate` and `exFactoryDate` (on `Shipment`) and `arrivalDate` (on `ShipmentConsignment`) are three of the four payment anchor events (`AnchorEvent` in `suppliers`; the fourth, `INVOICE`, is already live via 6.4). The **fields** are modelled now; **wiring the `AnchorEventDateKnownEvent` publish** when they're set is 7.2 (BL, ex-factory) and 7.6 (arrival), not 7.1. The BL fields are nullable so a record can exist before the BL is issued, but `blReference` is required before a provisional GRN (7.4's gate). `confirmedEtd` sits on the consignment for 7.5; the requested ETD stays on the PO — the delta is computed across the two, never duplicated.

### Flagged for the Product Owner batch

- **Partial shipment** — one PO spread across multiple containers. The model *permits* one PO to appear in several consignments (uniqueness isn't enforced), but the MVP workflow assumes **one consignment per PO**. Bundle this with the **partial-invoicing** question flagged in 6.4 — they're the same underlying commercial question (a split shipment strongly implies split invoicing), and their answers must be consistent.
- **What makes documents "in" enough to create the provisional GRN** (lands in 7.2/7.4) — all three documents, or BL + packing list with inspection optional? Lean: BL + packing list mandatory, inspection report recorded but not gating (an inspection can be pending while goods ship). A domain call, not yet resolved.
- The **container-fill deadline timezone** (parked below in Dates and timestamps) also belongs in this next batch, since Feature 8 follows straight after.

### Document logging & upload (Story 7.2)

The entry point of the whole shipment workflow, in `shipments`. Three multipart endpoints under `/api/purchase-orders/{poId}/shipment` — `bill-of-lading`, `packing-list`, `inspection-report` — each capturing **structured fields + the source file to S3**, plus `GET /shipment` (read) and `GET /shipment/documents/{type}` (download). `PURCHASING`/`ADMIN` to log; reads open.

- **Shipment creation is the first document.** `7.1` left open when a `Shipment` comes into existence; settled here: logging the first document for a PO creates the `Shipment` + its `ShipmentConsignment` together (no speculative empty records at PO-send time). Later documents reuse them. Attaching a *second* PO to an existing BL is 7.3. The PO must be `GENERATED`/`SENT` — `PO_NOT_READY_FOR_SHIPMENT` (new code) otherwise, same finalised-PO rule as PI/invoice logging.
- **Documents arrive incrementally, in any order.** Each endpoint logs independently against the consignment; none demands the others. BL reference/date + ex-factory are shipment-level; packing-list and inspection-report reference/date (+ inspection outcome) are per-consignment (V46 adds these structured columns). Receipt status stays `DOCUMENTS_PENDING` until 7.4's gate.
- **The anchor-date chain — the reason this story matters.** Setting the **BL date** publishes the `BL` anchor event, setting **ex-factory** publishes `EX_FACTORY`, through 6.2's seam via the 6.1 event pattern — **the first real callers besides 6.4's invoice date**. `ShipmentDocumentService` (the single "log a document" entry point, callable by the future AI feed) records via a separate-bean `ShipmentDocumentRecordingService` transaction that **commits first**, then publishes one `AnchorEventDateKnownEvent` **per affected PO** (the publishing loops over the shipment's consignments — trivially one here, so 7.3's co-loading gets the fan-out free). Best-effort, exactly as invoice logging: a publish failure never fails the recording. A **corrected date re-publishes** and 6.2's re-entrancy recalculates + audits. Verified end to end across three modules (BL logged → anchor event → 6.2 → balance due date set) in `ShipmentAnchorSeamTest`.
- **Corrections are audited, files are retained.** Re-posting a document type corrects it: a changed structured field records a `DOCUMENT_FIELD_CORRECTED` audit entry (old→new, who, when) — dates drive money timing, so no silent edits — and a replacement file is stored under a **fresh S3 key while the prior object is retained, never deleted** (`DOCUMENT_FILE_SUPERSEDED`). Append-only `shipment_document_audit_events` (V47), same construct-only/no-delete shape as 6.2/5.7. This is deliberately **lighter than PI/invoice supersession** — documents are evidence attachments, not versioned entities: audited field updates + retained files, not a superseded-row lifecycle.
- **Record faithfully, no cross-document verification.** Well-formedness only (non-blank references, a present BL date, a non-empty file, ISO dates parsed to a clean `VALIDATION_ERROR`). Whether the BL quantity matches the packing list is **not** checked — that's the AI layer's cross-document verification (Feature 10); the MVP stores files, never reads them. Cross-tenant PO → 404.
- New `shipments` cross-module dependencies, all via named interfaces (`ModularityTests` green): `purchaseorders` (`assertOwnPurchaseOrderReadyForShipment`, new), `payments` (publishes its `AnchorEventDateKnownEvent`), `suppliers` (`AnchorEvent`).

### Co-loaded containers (Story 7.3)

One BL carrying goods for multiple POs across multiple suppliers, operable. The 7.1 two-entity split made this structural; this story makes it a workflow. Endpoints under `/api/shipments/{shipmentId}/consignments`: `POST` (attach a PO), `DELETE /{poId}` (detach), `GET` (the "linked POs sharing this BL" listing). Attach/detach `PURCHASING`/`ADMIN`; listing open.

- **Per-consignment independence is the whole story.** Every rule — own packing list, own eligibility, own receipt, own anchor clock — exists so co-loaded POs move at their own pace. There is deliberately **no shipment-level "all documents in" gate**: that shortcut would recreate the exact failure the business rule prevents (one slow supplier freezing everyone's payments). The sibling-independence and document-isolation tests are the guardrails on that.
- **The receipting gate — the load-bearing rule.** *"Each linked PO requires its own packing list before its portion can be receipted."* Built as a named, reusable predicate `ShipmentConsignment#isReceiptEligible()` — true only when the consignment's **own** packing list is logged; a sibling's documents never satisfy it. **7.4 composes this** with whatever mandatory-document rule the POs settle (the still-flagged question); the packing-list-per-portion part is the piece the business rules state unambiguously, so it's built now.
- **Attach.** Preconditions: the PO is `GENERATED`/`SENT` and same-tenant (both via `assertOwnPurchaseOrderReadyForShipment` — cross-tenant 404s even when the shipment id is known), and not already consigned here (`PO_ALREADY_CONSIGNED`, new). The attached consignment starts `DOCUMENTS_PENDING` with empty document slots but **immediately inherits the shipment-level facts**: if the BL date already exists, the newly-attached PO's `BL` anchor publishes **on attachment** (7.2's publish loop, made explicit for the single new PO) — a BL-anchored supplier's balance clock starts the moment they join a dated BL. A BL-date **correction re-publishes for every consignment** on the shipment (7.2's re-entrancy, now genuinely plural — each affected PO's due date recalculates and audits).
- **Detach.** Allowed while `DOCUMENTS_PENDING` (a mis-linked PO is a realistic mistake), audited (`CONSIGNMENT_DETACHED`); **blocked once receipted** (`CONSIGNMENT_NOT_DETACHABLE`, new) — past provisional receipt it's an unwind, not a correction, a deliberate future feature if ever needed. **Soft-delete** (`detached` flag, V48) rather than a hard delete: the row and its audit trail are retained (evidence is never destroyed, same posture as a superseded file), and detached consignments are excluded from the active set (views, duplicate-attach check, the 7.2 document find-or-create). Re-attaching a previously-detached PO works.
- **Views.** The shipment listing gives each portion's PO/supplier/document-status/receipt-eligibility (reuses `PurchaseOrderService#getSummary` + `SupplierService#getSummary`). The PO's shipment view (7.2's `ShipmentResponse`) gained `coLoaded` + `coLoadedWithPurchaseOrderCount` so the response shape never assumes a single consignment. The anchor-publish loop was extracted to a shared `ShipmentAnchorPublisher` used by both document logging and attach.
- Verified end to end (`ShipmentCoLoadingAnchorTest`): co-load onto a dated BL starts the new PO's clock, and a BL-date correction moves **both** POs' due dates.

### Provisional GRN (Story 7.4) — the critical-path story

The provisional goods receipt: the record the three-way match (6.5) runs against, created once a consignment's documents are in, explicitly **without** requiring physical arrival (payment readiness keys off *documented* receipt, not physical landing). Landing this **unblocks 6.5, 6.6, 6.8**. Per-consignment, per 7.3's independence. Endpoints under `/api/purchase-orders/{poId}/shipment/provisional-grn`: `POST` (create), `PUT` (amend, pre-arrival), `GET` (read). Create/amend `PURCHASING`/`ADMIN`; read open.

- **Quantities are the substance.** A GRN that were only a status flag would let 6.5 "match" against nothing. The GRN records per-SKU **received quantities**, snapshotted from the **packing list**. This completed a 7.2 deferral: packing-list structured fields now include per-SKU line quantities (`shipment_packing_list_lines`, V49; full-replaced on packing-list logging; the packing-list endpoint gained a `lines` JSON param). The GRN lines (`shipment_goods_receipt_lines`, V51) are a **snapshot, not a live reference** — a later packing-list correction never silently rewrites an issued GRN (pinned by test); provenance principle shared with PO prices and payment amounts.
- **Explicit action, not automatic.** `POST` is a deliberate `PURCHASING`/`ADMIN` click that certifies "documents in and correct enough to receipt" — the human-in-the-loop; auto-creation on upload would let a typo'd packing list silently open the payment gate.
- **State + audit.** Transition `DOCUMENTS_PENDING → PROVISIONALLY_RECEIPTED` (arrival **not** required — pinned by test), recording who/when on the consignment (V50). Pre-arrival amendments are allowed with a required reason (old→new/who/why audited, `PROVISIONAL_GRN_AMENDED`); once arrival is confirmed (7.6) the GRN is settled (`PROVISIONAL_GRN_NOT_AMENDABLE`). Full 5.7-standard append-only audit.
- **Co-loaded siblings receipt independently** (7.3's independence carried through — tested, incl. under a rework hold).

#### Inspection-aware gating (revised — confirmed PO model)

The "which documents gate the GRN" question is now a **confirmed Product Owner decision** (not the earlier lean). "Correct document set" depends on whether the shipment is inspection-due and the inspection's outcome.

- **`inspection_due` flag** (`shipment_consignments`, V57) — settable/clearable per consignment (`PURCHASING`/`ADMIN`, `POST .../shipment/inspection-due`), audited; clearing waives a control so a **reason is required**. One internal operation (the seam the future Product-Risk × Factory-Performance scoring engine drives; the flag is the interface). **Never set = not due = the no-QC-service path** — a company that never touches inspection fields sees zero inspection behaviour anywhere.
- **Inspections are repeatable, latest governs.** `inspection_reports` (V58) is the ordered history, each with outcome (`PASS`/`REWORK_REQUIRED`/`FAIL`), date, S3 file, notes; the consignment caches the latest for the gate. The 7.2 inspection endpoint now takes an outcome enum + notes.
- **Three outcomes, distinguished by timing relative to shipment.** `PASS` → GRN clean. **`REWORK_REQUIRED` is a pre-shipment hold, not a discrepancy** — goods stayed at the factory, nothing shipped, **nothing to receive, no GRN**: the consignment moves to a new `ReceiptStatus.REWORK_REQUIRED` (V57 model), GRN creation hard-blocks (`CONSIGNMENT_IN_REWORK_HOLD`), and a subsequent `PASS` releases it back to `DOCUMENTS_PENDING` (both transitions audited). **`FAIL` (found post-shipment / shipped anyway) does not hold** — the GRN **still creates**, flagged `qc_failed`.
- **The gate (`ProvisionalGrnGate`) composes it all, one place:** own packing list + BL always; **not in rework hold**; if inspection-due, a resolved inspection (`PASS`/`FAIL`) must exist (`CONSIGNMENT_INSPECTION_PENDING` otherwise); if not due, no inspection requirement. Distinct codes per cause (packing-list/BL via `CONSIGNMENT_NOT_RECEIPT_ELIGIBLE`, `CONSIGNMENT_IN_REWORK_HOLD`, `CONSIGNMENT_INSPECTION_PENDING`). Re-create → `PROVISIONAL_GRN_EXISTS`.
- **GRN provenance** (`grn_provenance`, V57), keyed off the inspection outcome: `CLEAN` (passed), `INSPECTION_NOT_DUE` (no inspection outcome — created legitimately without one), or `QC_FAILED` (latest inspection failed). Surfaced on `GoodsReceiptResponse` (`provenance` + `qcFailed`).
- **`qc_failed` deliberately does NOT block anything downstream** (a *failed* inspection produces a *successful* GRN). The goods are physically real and on the water; blocking receipt would jam payment scheduling and stock over a quality dispute that belongs in the credit/dispute lane. It publishes a **`QcFailureEvent`** (payments-owned, `@NamedInterface("payment-events")`, the seam a future NCR/dispute flow hangs from — no NCR machinery built) and carries the flag; **the three-way match (6.5) proceeds unblocked** — `qc_failed` isn't a quantity/amount issue, so the GRN's quantities feed the match normally. This and the rework-is-not-a-failure rule are confirmed PO decisions, not defaults to improve on.

#### The Feature 7 → Feature 6 contract

The seam 6.5 binds to — the shape and the mechanism, worth pinning like 3.8's result was the Feature 3 → 4/5 contract:

- **Mechanism is push, not pull.** `shipments` already depends on `payments` (it publishes `AnchorEventDateKnownEvent`), so a `payments → shipments` pull would make the module graph cyclic (rejected by `ModularityTests`). So — exactly like the anchor seam — the GRN event is **owned by the consuming side** (`payments.event.ProvisionalGoodsReceiptEvent`, `@NamedInterface("payment-events")`) and **published by** `shipments`. 6.5 reacts to it; it never calls back into `shipments`.
- **Fat event.** It carries the received per-SKU quantities (`ProvisionalGoodsReceiptLine`) so 6.5 needs no follow-up read. Re-published on amendment (re-entrant, like the anchor seam), so revised quantities re-drive the match. Until 6.5 exists no one listens; publishing is a harmless no-op.
- **The read shape** (`GoodsReceiptResponse`): does a GRN `exists` for this PO (false while `DOCUMENTS_PENDING`), its `receiptStatus`, and the per-SKU `lines` (`skuId` + `receivedQuantity`) — the quantities 6.5 compares (PO qty = PI qty = **GRN qty**).

### Physical arrival confirmation (Story 7.6)

The second-stage action on a consignment (Screen 5's "Stage 2"): confirm goods physically arrived, checked **against the provisional GRN — never the original PO** — with a count mismatch creating a discrepancy record, never a reopened payment. Publishes the `ARRIVAL` anchor, completing the four-anchor set. Per consignment, addressed by its PO: `POST /api/purchase-orders/{poId}/shipment/arrival` (confirm), `PUT` (correct the arrival date), `GET` (read). `PURCHASING`/`ADMIN` to act; read open.

> **The governing principle — stated once, it governs the whole story: _arrival never unwinds settled state._** Not the payment, not the match, not the closure, not the GRN. Arrival produces exactly two things: a status, and (on mismatch) a discrepancy record pointing into the credit lane. Every tempting "shouldn't this also update…" is answered by the roadmap's own sentence: a discrepancy record, not a reopened payment.

- **The action.** Records the arrival date and the **arrived quantities per SKU** (what was counted at the door) on a `PROVISIONALLY_RECEIPTED` consignment. Preconditions: it must be provisionally receipted — no GRN, nothing to confirm against (`CONSIGNMENT_NOT_ARRIVAL_ELIGIBLE`) — and not already confirmed (`ARRIVAL_ALREADY_CONFIRMED`).
- **The comparison is strictly vs the GRN snapshot, per SKU — the PO is not consulted.** This is the deliberate design stance: the payment already matched against the provisional receipt (6.5), so the GRN is the settled expectation; re-deriving from the PO here would re-litigate what the match settled — precisely what "not a reopened payment" forbids. (A PO shortfall was either already visible at receipting — a partial shipment, handled by rollup/closure — or is a shortfall against the GRN too.)
- **Match** (all SKUs equal) → `PROVISIONALLY_RECEIPTED → ARRIVED_CONFIRMED`, audited. **Mismatch** (any SKU differs, short *or* over) → `ARRIVED_WITH_DISCREPANCY` + an **`ArrivalDiscrepancy`** record (per-SKU expected(GRN)/arrived/direction). **No payment status changes. No match re-runs.** A `PAID` payment provably stays `PAID` through a discrepant arrival (tested) — the delta is a money conversation in the credit lane (6.7: a shortfall's natural resolution is a `SHORT_SHIPMENT` credit).
- **The discrepancy record is shipments-owned, deliberately not a payments `DiscrepancyCase`.** A match discrepancy and an arrival discrepancy resolve *differently* (override/dispute-a-payment vs credit-for-shortfall) and must not be conflated — the same reason `qc_failed` and a quantity discrepancy stay two records. It surfaces to the resolver/credit lane via a payments-owned `ArrivalDiscrepancyEvent` (the seam, following `QcFailureEvent` exactly — no consumer built yet); resolution flows through the existing credit-ledger endpoint.
- **The `ARRIVAL` anchor** publishes on confirmation (**either outcome** — goods arrived regardless of counts), per affected PO, through the 6.2 seam; arrival-anchored balance due dates then calculate. A corrected arrival date (`PUT`) re-publishes; 6.2's re-entrancy recalculates. Per-consignment: on a co-load, each consignment confirms and anchors independently — one PO's goods counted-in while a sibling's still sit at the port.
- **`qc_failed` consignments confirm normally** — the quality lane and the arrival count are independent facts. An arrival mismatch on a `qc_failed` consignment creates its *quantity* discrepancy **alongside** the existing quality one — two causes, two records against the same consignment; they don't merge (they resolve differently).
- **GRN amendments are blocked post-arrival** (7.4's `PROVISIONAL_GRN_NOT_AMENDABLE` — the GRN is settled history once arrival is confirmed); corrections flow to the discrepancy/credit lane.
- **Cumulative receipt & closure are unaffected by arrival deltas — pinned.** The rollup/closure operation counts **GRN quantities**, which remain the receipt truth (the match and closure already ran on them — the "not reopened" principle applied consistently); an arrival shortfall lives purely as the discrepancy/credit. A **closed PO stays closed through an arrival shortfall** (tested). The alternative — arrival adjusting cumulative receipt and reopening closure — would re-litigate settled state and cascade into matched invoices, which the roadmap's own rule forbids.

### ETD tracking (Story 7.5) — the last story in Feature 7

Record the supplier's **confirmed ETD** against the PO's **requested ETD** (4.4, fixed post-generation), surface the signed delta, and keep the history when it shifts. The data foundation Phase 2's proactive-chasing feature will consume. In `shipments`. `PUT /api/purchase-orders/{poId}/shipment/etd` (set/revise, `PURCHASING`/`ADMIN`), `GET` (the delta + history, open read). The raw `confirmedEtd` also appears on `ConsignmentResponse`.

- **ETD is not an anchor — no payment interaction, deliberately.** An estimated departure informs logistics, not due dates; this story publishes no anchor event and is **provably inert beyond its own fields** (asserted by test — a set ETD leaves an arrival-anchored balance untouched, no due date appears). Stated here because it's the wire someone will want to cross.
- **First shipment contact.** A confirmed ETD can arrive weeks ahead of any document, so — unlike most consignment data — it doesn't gate on document state, and logging one **counts as first shipment contact**: it creates the shipment/consignment via the shared `ConsignmentProvisioningService` (7.2's create-on-first-touch path, extracted in 7.5 so document logging and ETD logging use one path). Settable from any state **before arrival is confirmed** — an ETD after the goods have landed is meaningless (`ETD_NOT_SETTABLE_AFTER_ARRIVAL`).
- **Revisions are the norm, kept as history.** ETDs slip; every change appends an `EtdRevision` (date, who, when, optional free-text reason — optional because demanding one for every routine slip is friction, but the field lets meaningful ones be recorded). The current value is the latest entry (mirrored on the consignment for easy read); the trail is the story — the substrate a supplier-reliability picture reads without anyone deciding to track it. The **history is the audit** (no separate audit-event type).
- **The delta — derived, directional, null-safe.** `delta = confirmed − requested`, signed days: **positive = later than requested (the slip)**, negative = earlier. Derived at read time from the two source dates, never stored (either can change — the drift rule). No confirmed ETD → null delta, shown honestly as `awaitingConfirmation`, never a fake zero. Served by the dedicated `GET .../shipment/etd` (with `requestedEtd`, read cross-module via `PurchaseOrderService#getRequestedEtd`) — a standalone read the frontend composes onto the consignment, shipment, and **PO** screens, the same pattern as the running position / receipt rollup / match results (a Purchasing user sees the slip on their PO screen without opening the shipment record).
- **Deliberately not built:** no slip alerts / chasing / thresholds (nobody has defined "slipped more than N days" — the history supports any future rule without rework; Phase 2 / Notifications), no carrier/vessel tracking (Phase 2 freight), no anchor or payment behaviour, no requested-ETD changes (that's a PO field, locked post-generation per 4.4 — the *confirmed* side is the moving one).
...
| Confirmed ETD | `LocalDate` | Model in place — Story 7.1, `ShipmentConsignment#confirmedEtd` (the delta vs. PO's requested ETD is 7.5) |
| Ex-factory date (the `EX_FACTORY` anchor) | `LocalDate` | Implemented — Story 7.2 publishes it, `Shipment#exFactoryDate` |
| BL date (the `BL` anchor) | `LocalDate` | Implemented — Story 7.2 publishes it, `Shipment#blDate` |
...
- PO-issuance gate (wires the supplier remodel into Feature 4; confirmed PO precondition tiers): the PO now requires a **validated supplier** and **incoterms**, carries **advisory flags** for contract/compliance, and holds **informational** delivery address + budget code. **Two-tier gating, deliberately:** the validated-supplier check bites at **draft creation** (`SUPPLIER_NOT_VALIDATED` at `POST /purchase-orders` — fail fast, not after 20 lines of entry) **and is re-checked at generation** (a supplier reverting to PENDING via the bank-details-change control blocks generation until re-validated); **incoterms** bite at **generation** (nullable in draft, required to generate — added to 4.6's composed precondition set, `PO_NOT_READY_TO_GENERATE`). Incoterms are an Incoterms-2020 enum with a **per-supplier default** (`suppliers.default_incoterms`, V62) pre-filled on draft creation and per-PO editable (`PUT .../details`), rendered on the PO document. **Advisory (never-blocking) flags** stamped at generation: `contract_pending` (no contract reference) / `compliance_pending` (supplier compliance not confirmed), persisted (V63), surfaced in PO views, audited (append-only `purchase_order_audit_events`, V64); they clear when the reference/cert lands (`POST .../advisory-flags/refresh`, audited). **Informational:** delivery address defaults from the company profile (`companies.default_delivery_address`, V62, read cross-module via the new onboarding `CompanyDefaultsService` `@NamedInterface`) and is on the document; budget code is inert free-text. Priced-lines/agreed-terms preconditions were already enforced (4.2/4.6). New cross-module surface `SupplierService#getIssuanceView` (`@NamedInterface`, validated/complianceConfirmed/defaultIncoterms — booleans/string, never the suppliers enums); new `purchaseorders → onboarding` dep, all via named interfaces. New code `SUPPLIER_NOT_VALIDATED`. `ModularityTests` green; full `mvn test` 472.
- Supplier-record remodel (confirmed PO terms model + validation lifecycle, one migration pass): reworked 3.3's terms and extended 3.1's supplier. **Terms:** flat record → typed `current`/`target` term records (`terms_type` ZERO_DEPOSIT/DEPOSIT_BALANCE/ROLLING, nullable 1dp `deposit_pct`, five-value anchor incl. `STATEMENT_DATE`, signed `days_from_anchor`); type-consistency enforced (`INVALID_TERMS_COMBINATION`); explicit audited `ADMIN`/`FINANCE` target-activation promoting + retaining history (`NO_TARGET_TERM` if none); ROLLING storable (behaviour = 6.5 re-spec). Migration V59 transforms in place (id=supplier_id trick, portable), pre-existing PO snapshots proven unmoved. The 6.2 resolver reads `current` with **no caller change** (the isolation point paying off). **Validation lifecycle:** new `validation_status` PENDING/VALIDATED (distinct from ACTIVE/INACTIVE); bank details stored, **masked in the default response (last 4), full read FINANCE/ADMIN-only**; **a bank-details change reverts VALIDATED→PENDING, loudly audited** (the invoice-fraud control — friction on purpose); compliance status; derived `readyForValidation` vs explicit FINANCE/ADMIN `validate` (only-when-ready, `SUPPLIER_NOT_READY_FOR_VALIDATION`) / `unvalidate`-with-reason; append-only `supplier_audit_events` (V61). V60 migrates supplier fields; **validation default: already-ordered suppliers → VALIDATED, the rest PENDING**. What validation gates (PO issuance) is the next story. New codes `INVALID_TERMS_COMBINATION`/`NO_TARGET_TERM`/`SUPPLIER_NOT_READY_FOR_VALIDATION`. `ModularityTests` green; full `mvn test` 466.
...
- Feature 7, Story 7.4 **revised** (inspection-aware gating — confirmed PO model): reworked the Provisional GRN subsection. The "which documents gate" question is now **settled** (was a flag): BL + packing list always; the inspection report mandatory only when **inspection-due** (`inspection_due` flag, V57 — settable/clearable-with-reason, one internal op = the future scoring engine's seam; never-set = the no-QC path). Inspections are now **repeatable** (`inspection_reports`, V58, latest governs; the 7.2 endpoint takes an outcome enum + notes). **Three outcomes distinguished by timing:** `PASS` → clean; **`REWORK_REQUIRED` is a pre-shipment hold, NOT a discrepancy** — new `ReceiptStatus.REWORK_REQUIRED`, nothing shipped so no GRN (`CONSIGNMENT_IN_REWORK_HOLD`), released by a later `PASS` (both audited); **`FAIL` still creates the GRN** flagged `qc_failed`. Gate (`ProvisionalGrnGate`) composes packing-list + BL + not-in-rework + inspection-resolved-when-due (`CONSIGNMENT_INSPECTION_PENDING`); GRN carries `grn_provenance` (V57: `CLEAN`/`INSPECTION_NOT_DUE`/`QC_FAILED`) on `GoodsReceiptResponse`. **`qc_failed` deliberately does NOT block the match** — publishes a `QcFailureEvent` (payments-owned seam, no NCR machinery) and the receipt/match proceed (a failed inspection → a successful GRN; the quality dispute runs in the credit/dispute lane). Per-consignment throughout (rework hold on one doesn't stop a sibling — tested). New codes `CONSIGNMENT_IN_REWORK_HOLD`/`CONSIGNMENT_INSPECTION_PENDING`; `CurrentUserContext#getOrNull` reused for nullable actors. `ModularityTests` green; full `mvn test` 467 (6 new). *(Original 7.4 entry below superseded by this.)*
- Feature 7, Story 7.4 (critical path — **unblocks 6.5/6.6/6.8**): added the Provisional GRN subsection + the Feature 7 → 6 contract. The provisional goods receipt the three-way match runs against, created once documents are in, explicitly **without** physical arrival. Per-consignment; endpoints under `/api/purchase-orders/{poId}/shipment/provisional-grn` (`POST` create / `PUT` amend / `GET` read). **Quantities are the substance** — the GRN snapshots per-SKU received quantities from the packing list, which forced completing a 7.2 deferral: packing-list line quantities now captured (`shipment_packing_list_lines`, V49; `lines` JSON param on the packing-list endpoint); GRN lines (`shipment_goods_receipt_lines`, V51) are a **snapshot** (a later packing-list correction never rewrites an issued GRN — tested). **Explicit `PURCHASING`/`ADMIN` action**, not auto-on-upload (the human-in-the-loop). **Gate isolated in `ProvisionalGrnGate`** (PO question #1's one-line flip point): 7.3's own-packing-list rule + BL (reference+date); **inspection non-blocking** per the stated lean; blocked create names the missing doc (`CONSIGNMENT_NOT_RECEIPT_ELIGIBLE`), re-create `PROVISIONAL_GRN_EXISTS`. Transition `DOCUMENTS_PENDING → PROVISIONALLY_RECEIPTED` (arrival not required — pinned; who/when on consignment, V50); pre-arrival amend allowed with required reason (audited, `PROVISIONAL_GRN_AMENDED`), post-arrival settled (`PROVISIONAL_GRN_NOT_AMENDABLE`). **The Feature 7 → 6 seam is push (event), not pull** — `shipments→payments` already exists via the anchor event, so a pull would cycle; the fat `ProvisionalGoodsReceiptEvent` (owned by `payments`, `@NamedInterface`, carrying `ProvisionalGoodsReceiptLine`s) is published by `shipments` and re-published on amend. Sibling receipt-independence + the event + snapshot-immutability all tested (`ProvisionalGrnControllerTest`, using `@RecordApplicationEvents`). `ModularityTests` green; full `mvn test` 431 (10 new).
- Feature 7, Story 7.3: added the Co-loaded containers subsection — one BL across multiple POs/suppliers made operable, in `shipments`. Endpoints under `/api/shipments/{shipmentId}/consignments`: attach a PO (`POST`), detach (`DELETE /{poId}`), list (`GET` — the "linked POs sharing this BL" view with per-portion PO/supplier/doc-status/eligibility). **Per-consignment independence is the whole story** — own packing list, own eligibility, own receipt, own anchor clock; **no shipment-level "all documents in" gate** (that shortcut recreates the failure the rule prevents). The load-bearing rule is a named reusable predicate `ShipmentConsignment#isReceiptEligible()` (true only when the consignment's **own** packing list is logged) — **7.4 composes it** with the still-flagged mandatory-documents question. Attach: PO must be `GENERATED`/`SENT` + same-tenant + not already consigned here (`PO_ALREADY_CONSIGNED`, new); on attach to an **already-dated** BL the new PO's `BL` anchor publishes immediately (its balance clock starts), and a BL-date **correction re-publishes for every** consignment (7.2's loop, now genuinely plural — each PO's due date recalcs + audits). Detach: allowed + audited while `DOCUMENTS_PENDING`, blocked once receipted (`CONSIGNMENT_NOT_DETACHABLE`, new); **soft-delete** (`detached` flag, V48) retaining the row + audit trail, excluded from the active set (re-attach works). Extracted the anchor-publish loop to a shared `ShipmentAnchorPublisher`; `ShipmentResponse` gained `coLoaded`/`coLoadedWithPurchaseOrderCount`; two new audit types (`CONSIGNMENT_ATTACHED`/`DETACHED`). Document isolation + sibling receipt-independence proven by test; the anchor-on-attach + republish-to-all chain integration-tested across three modules (`ShipmentCoLoadingAnchorTest`). `ModularityTests` green; full `mvn test` 421 (9 new).
- Feature 7, Story 7.2: added the Document logging & upload subsection — the entry point of the shipment workflow, in `shipments`. Three multipart endpoints (`bill-of-lading`/`packing-list`/`inspection-report`) under `/api/purchase-orders/{poId}/shipment` capture structured fields + the source file to S3, plus read + document-download endpoints; `PURCHASING`/`ADMIN` to log. **Logging the first document creates the shipment + consignment** (7.1's open entry-point decision, settled); the PO must be `GENERATED`/`SENT` (`PO_NOT_READY_FOR_SHIPMENT`, new code). Documents log independently in any order; V46 adds packing-list/inspection structured columns; V47 adds the append-only `shipment_document_audit_events`. **The anchor-date chain is the headline**: setting the BL/ex-factory date publishes `BL`/`EX_FACTORY` via 6.2's seam (the first callers besides 6.4's invoice) — `ShipmentDocumentService` (the single entry point, AI-feed-ready) commits via a separate-bean recording transaction, then publishes one `AnchorEventDateKnownEvent` per affected PO (loop over consignments, so 7.3 co-loading gets the fan-out free); corrections re-publish and 6.2 recalculates + audits; best-effort like invoice logging. Corrections audited (`DOCUMENT_FIELD_CORRECTED` old→new), replacement files stored under a fresh key with the prior retained (`DOCUMENT_FILE_SUPERSEDED`) — deliberately lighter than PI/invoice supersession (evidence attachments, not versioned entities). Record-faithfully: no cross-document verification (AI layer, Feature 10); files stored, never read. New `shipments`→`purchaseorders`/`payments`/`suppliers` deps, all via named interfaces. The full BL→anchor→due-date chain is integration-tested across three modules (`ShipmentAnchorSeamTest`). `ModularityTests` green; full `mvn test` 412 (11 new).
- Feature 7, Story 7.1 (first story of the feature): added the Shipment & Goods Receipt section — `Shipment` (BL-level, V44) + `ShipmentConsignment` (per-PO portion, V45), model only, both `TenantScoped`, related one-to-many via a loose `shipmentId` UUID (no JPA association, same convention as every entity here). **The two-entity split is the whole story**: the co-loading rule (one BL, many POs across many suppliers, each portion with its own packing list and independent receipting) makes BL-level and per-PO genuinely different concepts — a non-co-loaded shipment is just a shipment with one consignment (no special-casing), and co-loaded is derived (consignment count > 1), never stored. BL/ex-factory dates live on the shipment, packing-list/inspection-report keys + confirmed ETD + arrival date + `receiptStatus` on the consignment (per-consignment, not per-shipment). `ReceiptStatus` enum (`DOCUMENTS_PENDING`→`PROVISIONALLY_RECEIPTED`→`ARRIVED_CONFIRMED`/`ARRIVED_WITH_DISCREPANCY`) defined now, **transitions not yet enforced** (7.4/7.6). Anchor dates (BL, ex-factory, arrival) are modelled but **not published** — wiring `AnchorEventDateKnownEvent` is 7.2/7.6; with 6.4's `INVOICE` already live, all four anchor fields will then exist. **No upload flow, no anchor publishing, no GRN, no arrival logic** — 7.2–7.6. A load-bearing test proves co-loading spans two suppliers within a company yet never crosses the company boundary. Settled the Confirmed/ex-factory/BL/arrival date rows in the Dates table. Two new PO-batch flags: **partial shipment** (model permits PO→many consignments, MVP assumes one — bundled with 6.4's partial-invoicing question, same commercial call) and **what makes documents "in"** for the provisional GRN (7.2/7.4). `ModularityTests` green (`shipments` gained no cross-module dependency); full `mvn test` 401 (7 new).
- Feature 6, Story 6.5 (the pre-payment control the product exists for; **opened 6.6/6.8**): added the Three-way match evaluation subsection + the Feature 5/7 → 6 seam. Before a **balance** is `READY_TO_PAY`, PO = confirmed PI = GRN vs the final invoice must agree — pass → `READY_TO_PAY`, any fail → `BLOCKED` (**block by default**). Four legs: PO (pulled), **confirmed** PI (`AUTO_CONFIRMED`/`APPROVED` only — new `ProformaInvoiceMatchService` `@NamedInterface`), provisional-or-confirmed GRN (projected, see seam), active invoice + claimed credit. Checks: quantity chain PO=PI=GRN per SKU; **money equality exact 2dp, no tolerance** (deliberate asymmetry with reconciliation — a penny is a fail); claimed credit validated via 6.7 (`checkClaim` → deduct + `APPLIED` on pass, unmatched → fail). **Reference basis decided (b)**: PI prices × **GRN** quantities − validated credit, so the GRN leg means something (catches short-shipped-but-fully-invoiced); isolated in the pure `ThreeWayMatchEvaluator`. **Event-driven** (`MatchTriggerListener`, all `@TransactionalEventListener(AFTER_COMMIT, fallbackExecution=true)`): PO generated, PI confirmed (new `reconciliation`-owned `ProformaInvoiceConfirmedEvent`), GRN event (7.4), invoice/credit (internal `MatchInputChangedEvent`); deterministic, every evaluation audited (`MATCH_PASSED`/`MATCH_BLOCKED` + `match_detail` V53), missing legs = honest `PENDING` "awaiting X" (not `BLOCKED`), re-entrant credit handling, human `PAID`/`ON_HOLD` never overridden. **Deposit gate flagged + isolated** (`PaymentGatePolicy`): match gates the balance, deposit payable without it (literal rule would deadlock deposits). **The Feature 5/7 → 6 seam is push, not pull** to keep the module graph acyclic: GRN is projected from 7.4's event into `payment_grn_projection_lines` (V52) because `shipments→payments` already exists; the confirmed-PI trigger is a `reconciliation`-owned event + a `payments→reconciliation` pull (no prior dependency). New `Payment` transition methods (guarded vs human states); `MATCH_*`/`DEPOSIT_PAYABLE` audit types. `ModularityTests` green (`payments→reconciliation` acyclic); full `mvn test` 450 (19 new). **6.6 (mismatch routing) and 6.8 (release) now open.**
- Feature 6, Story 6.6 (the human half of the match control): added the Mismatch routing & resolution subsection. When 6.5 blocks a payment, a **discrepancy case** (`discrepancy_cases` V54 + append-only audit V55) is opened idempotently from the match (`onMatchBlocked`/`onMatchPassed` hooks — one case per blocked payment, a passing re-run auto-resolves it) and notified via the `EmailSender` seam (**4th consumer**, new onboarding `UserDirectoryService#resolveDiscrepancyResolverEmails` `@NamedInterface`). Endpoints under `/api/discrepancies`: queue/stats/side-by-side reads + `claim`/`credit`/`override`/`dispute`. **Routing = claimable queue** (flagged interpretation of "named resolver"). **Side-by-side served whole** — every leg + the PO's open ledger entries. **Four resolution paths:** correct→auto-resolve (`CORRECTED`), agree a credit from the case (`CREDITED`, links case→ledger entry, resolves when the match passes), **override** (force-pass `READY_TO_PAY`, required reason, `match_overridden` V56 so a re-run can't re-block — **`FINANCE`/`ADMIN` only, flagged**), **dispute** (`DISPUTED`, payment stays `BLOCKED`, the NCR seam). **Dashboard stat pinned:** "Open discrepancies" = open discrepancy *cases* (`GET /api/discrepancies/stats`), correcting 6.7's placeholder (whose open-entry count is a separate "open credits" ledger view). New `DISCREPANCY_NOT_OPEN` code; `CurrentUserContext#getOrNull` for system audit actors. **Fixed a latent bug:** `@TransactionalEventListener(AFTER_COMMIT)` + `REQUIRED` writes silently don't commit — the credit/PI-confirm/PO-generation match triggers were no-ops in production (only the invoice trigger, published outside a tx, worked); `ThreeWayMatchService#evaluate` + `GrnProjectionService#project` are now `REQUIRES_NEW`. New `payments → onboarding` dep (named interface); `ModularityTests` green; full `mvn test` 461 (11 new). **6.8 (release) is the last built story of Feature 6.**
