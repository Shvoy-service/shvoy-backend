...
**Owner:** Story 5.1 (PI data model), 5.2 (log a supplier's confirmed PI), 5.3 (variance calculation), 5.4 (tolerance evaluation & auto-confirm), 5.6 (approver pool configuration — built in `onboarding`, see below). Still to come: the 2-of-N sign-off gate (5.5) and the full status lifecycle/audit trail (5.7) — see "Still to come" below.
...
**Status:** nothing in this subsection is built yet — these are decisions the Product Owner has already confirmed, recorded now so they aren't lost before 5.5+ actually builds against them. Treat this the same as the Phase 2 section below: a landing place for settled decisions about future work, not a spec for current behaviour.

**Multi-currency PI — reject-and-route, not a hard validation error.** When a supplier's PI arrives in a different currency than its PO, Feature 5 must **not** treat this as a blunt `400`/`VALIDATION_ERROR`. Instead it routes to an approver as an exception, and **does not auto-convert** (no FX; Phase 2's FX-rate machinery is separate). **Built through 5.4:** 5.3 flags the mismatch on the reconciliation record and 5.4 routes on it (`CURRENCY_MISMATCH`) — the approval mechanics that consume the routed PI are 5.5/5.6.

**Nothing open blocks the reconciliation core anymore.** The variance basis (unit price) and the ±2% tolerance boundary (exclusive, per-account 2%) are both resolved and built — pending only Product Owner confirmation the two defaults are right, which is a value change, not a rebuild. With 5.6 (the approver pool) also built, **5.5 (the 2-of-N sign-off gate) has all its inputs**: the routed PIs (5.4), the eligible-approver set and required count (5.6), and the increase/decrease direction (`VarianceDirection`, 5.3) — plus the runtime-robustness flag in the Approver pool section to handle a pool shrunk below its count after routing.
