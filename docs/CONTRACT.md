...
**Owner:** Story 5.1 (PI data model), 5.2 (log a supplier's confirmed PI), 5.3 (variance calculation), 5.4 (tolerance evaluation & auto-confirm), 5.5 (approval routing & the 2-of-N sign-off), 5.6 (approver pool configuration — built in `onboarding`, see below). Still to come: the full status lifecycle/audit trail (5.7) — see "Still to come" below.
...
**Status:** only 5.7 remains in Feature 5 — these are decisions already confirmed, recorded so they aren't lost before it builds against them. Treat this the same as the Phase 2 section below: a landing place for settled decisions about future work, not a spec for current behaviour.

**Multi-currency PI — reject-and-route, not a hard validation error.** When a supplier's PI arrives in a different currency than its PO, Feature 5 must **not** treat this as a blunt `400`/`VALIDATION_ERROR`. Instead it routes to an approver as an exception, and **does not auto-convert** (no FX; Phase 2's FX-rate machinery is separate). **Built end to end:** 5.3 flags the mismatch on the reconciliation record, 5.4 routes on it (`CURRENCY_MISMATCH`), and 5.5's single-approver path resolves the routed PI (a currency mismatch alone, with no price increase, doesn't invoke the 2-of-N gate).

**5.7 (status lifecycle & audit) is the last Feature 5 story.** 5.1–5.6 each transition the PI `status` and write immutable records (`Reconciliation`, `ApprovalAction`) as they go; 5.7 threads those states into one coherent lifecycle (legal transitions, guards) and a unified audit view. The reconciliation core itself is complete and has no open questions — the variance basis (unit price) and the ±2% tolerance boundary (exclusive, per-account 2%) are both built, pending only Product Owner confirmation the two defaults are right (a value change, not a rebuild).
