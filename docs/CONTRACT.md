...
**Owner:** Story 4.1 (PO data model), first story of Feature 4. Data model only — no create/edit endpoints, no pricing computation, no generation/sending. `PurchaseOrder`/`PurchaseOrderLine` are reachable only via direct repository access until 4.4+.
...
- `PurchaseOrderLine` — tenant-scoped, linked to its `PurchaseOrder` and a `sku_id`, `line_number` (stable display order) and `quantity` (both required at creation, whenever 4.4 adds the endpoint that creates one), plus four **price snapshot** fields.
...
All four snapshot fields are **nullable, with no mutator yet** — this story defines the columns; populating them is 4.2 (resolve via `PriceResolutionService`) and 4.3 (compute the total). Same "column exists before the story that fills it" shape as `Sku.carton_size` existing before any endpoint set it (3.7). `PurchaseOrderLine#getUnitPrice()`/`#getLineTotal()` compose `UnitPrice`/`Money` from the snapshot columns, returning null until they're set — same convenience-view pattern as `SkuPrice#getUnitPrice`.
