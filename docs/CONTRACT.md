...
**Status:** nothing in this subsection is built yet — these are decisions the Product Owner has already confirmed, recorded now so they aren't lost before 5.3+ actually builds against them. Treat this the same as the Phase 2 section below: a landing place for settled decisions about future work, not a spec for current behaviour.
